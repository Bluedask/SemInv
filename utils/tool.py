...
def load_models(arch_name,model_filepath,device):
    ...
    if arch_name == 'distilbert':
        backbone_filepath = os.path.join(tool.TROJAI_R6_DATASET_DIR,'embeddings/DistilBERT-distilbert-base-uncased.pt')
        backbone_model = torch.load(backbone_filepath).to(device)
        tokenizer = transformers.DistilBertTokenizer.from_pretrained('distilbert-base-uncased')
        benign_reference_model_filepath = os.path.join(tool.TROJAI_R6_DATASET_DIR,'models/id-00000006/model.pt')
        benign_model = torch.load(benign_reference_model_filepath).to(device)
    elif arch_name == 'gpt2':
        backbone_filepath = os.path.join(tool.TROJAI_R6_DATASET_DIR,'embeddings/GPT-2-gpt2.pt')
        backbone_model = torch.load(backbone_filepath).to(device)
        tokenizer = transformers.GPT2Tokenizer.from_pretrained('gpt2')
        benign_reference_model_filepath = os.path.join(tool.TROJAI_R6_DATASET_DIR,'models/id-00000001/model.pt')
        benign_model = torch.load(benign_reference_model_filepath).to(device)
